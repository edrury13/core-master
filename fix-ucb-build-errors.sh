#!/bin/bash
# Script to fix UCB build errors

echo "Fixing UCB build errors..."
...
# First, let's check if we need to include curl differently
cd "$WSL_DEST"

# Fix 1: Add curl includes to gdocs_auth.cxx
echo "Adding curl includes..."
sed -i '/#include "gdocs_auth.hxx"/a #include <curl/curl.h>' "$WSL_DEST/ucb/source/ucp/gdocs/gdocs_auth.cxx"

# Fix 2: Update gdocs_authservice.cxx to fix namespace and password container issues
cat > "$WSL_DEST/ucb/source/ucp/gdocs/gdocs_authservice_fixed.cxx" << 'EOF'
...
namespace {
    ...
    {
        OUString sClientId;
        OUString expandedPath;
        osl::FileBase::getSystemPathFromFileURL(
            u"${BRAND_BASE_DIR}/program/google_client_id.txt"_ustr, expandedPath);
        
        osl::File aFile(expandedPath);
    ...
    OUString getGoogleClientSecret()
    {
        ...
        OUString expandedPath;
        osl::FileBase::getSystemPathFromFileURL(
            u"${BRAND_BASE_DIR}/program/google_client_secret.txt"_ustr, expandedPath);
...
void GoogleDriveAuthService::storeTokens(const AuthTokens& tokens)
{
    ...
    try
    {
        ...
        if (xContainer.is())
        {
            // Create password sequence
            uno::Sequence<OUString> aPasswords{ tokens.access_token };
            ...
            uno::Sequence<OUString> aRefreshPasswords{ tokens.refresh_token };
            xContainer->addPersistent(
                sRefreshUrl,
                tokens.user_email,
                aRefreshPasswords,
        ...
        if (xContainer.is())
        {
            ...
            for (const auto& record : aRecords)
            {
                if (record.Url == GDRIVE_AUTH_URL && record.UserList.hasElements())
                {
                    ...
                    if (userRecord.Passwords.hasElements())
...
mv "$WSL_DEST/ucb/source/ucp/gdocs/gdocs_authservice_fixed.cxx" "$WSL_DEST/ucb/source/ucp/gdocs/gdocs_authservice.cxx"

echo "Done fixing authservice!"