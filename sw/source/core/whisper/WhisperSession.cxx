...
public:
    ...
    void transcribe(const std::vector<sal_uInt8>& rAudioData,
        ...
        try {
            ...
            SAL_WARN("sw.whisper", "WhisperSession::transcribe - API key configured: " 
                     << (m_rConfig.hasApiKey() ? "YES" : "NO"));
            ...
            // Set CA bundle path for WSL/Linux
            #ifdef __linux__
            // Try common CA bundle locations
            const char* caBundlePaths[] = {
            ...
            for (int i = 0; caBundlePaths[i] != nullptr; ++i) {
                if (access(caBundlePaths[i], R_OK) == 0) {
                    curl_easy_setopt(curl, CURLOPT_CAINFO, caBundlePaths[i]);
                    SAL_WARN("sw.whisper", "Using CA bundle: " << caBundlePaths[i]);
                    foundCABundle = true;
                    break;
                }
            }
            
            // If no CA bundle found and environment variable is set, disable SSL verification (TESTING ONLY)
            if (!foundCABundle) {
                const char* disableSSL = getenv("LIBREOFFICE_WHISPER_DISABLE_SSL_VERIFY");
                if (disableSSL && strcmp(disableSSL, "1") == 0) {
                    SAL_WARN("sw.whisper", "WARNING: Disabling SSL verification for testing - DO NOT USE IN PRODUCTION!");
                    curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
                    curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
                } else {
                    SAL_WARN("sw.whisper", "No CA bundle found. Install ca-certificates or set LIBREOFFICE_WHISPER_DISABLE_SSL_VERIFY=1 for testing");
                }
            }
            #endif
